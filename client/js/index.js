let indexModule = (function () {
	...
	return {
		async init() {
			 ...
			 *    2.如果本地做了数据缓存（1MIN），当用户信息或者权限修改后，可能导致本地的数据依然是之前的，而且当用户重新退出登录，再进入首页，我们按理应该是重新获取最新用户的信息，而不是之前用户的信息，但是如果是从本地获取，很可能获取的还是上一个用户的信息； =>在退出登录的时候（或者重新登录的时候）我们应该把之前存储的信息都删除掉；当用户信息发生更改，应该让用户重新退出登录...
			 */
		}
	};
})();
